...
var RoudlerConfig = {
    ...
    globalPath:"",
    globalPackadePath:"sandbox/Roudler-CS1/roudler-client/"
}
////////////// Init Roudler ///////////////
Roudler = (function() {
    //Object Declaration
    ...
    Object.defineProperty(_self, "version",{value:0.02,writable:false,enumerable:false,configurable:false});
...
RoudlerConfig.globalPath = getAbsolutePath();
    ...
    //Global Private
    var roudlerModules = [], roudlerPollyfills = [], roudlerInjectedScripts = [], roudlerLoadedStyles = [];
    ...
    var globalModules = [RoudlerConfig.globalPackadePath+"roudler-elements/elements-native.js", "roudler-client/roudler-data/data-native.js", RoudlerConfig.globalPackadePath+"roudler-async/async-init.js", RoudlerConfig.globalPackadePath+"roudler-specification/specification-init.js"];
    var standardModules = {es6:[{type:"module",path:RoudlerConfig.globalPackadePath+"roudler-elements/elements-init.js",file:RoudlerConfig.globalPackadePath+"roudler-elements/elements-init-old.js"},{type:"text/javascript",path:"roudler-client/roudler-animations/animations-init-old.js"}],es5:["roudler-client/roudler-elements/elements-init-old.js", "roudler-client/roudler-animations/animations-init-old.js"]};
    var preloadedModules = {include:{es5:[],es6:[],es7:[],es8:[],native:[],other:[]},require:{es5:[],es6:[],es7:[],es8:[],native:[],other:[]}};
    ...
    _self.pollyfills = function() {return [].concat(roudlerPollyfills);}
    _self.loadedDynamicallyStyles = function() { return [].concat(roudlerLoadedStyles); }
    _self.injectStyle = function(hrefn, type=0) {
        var startTime = performance.now();
        if(type==0) {
        styleLoad = document.createElement("link");
        styleLoad.setAttribute("rel", "stylesheet");
       // if(hrefn.indexOf(".")==0) scriptLoad.src = srcn; else scriptLoad.src = _self.location.globalpath+"/"+srcn;
        styleLoad.setAttribute("href", hrefn);
        } else if(type==1) {
        styleLoad = document.createElement("style");
        styleLoad.textContent = hrefn;
        } else { console.error("Cannot inject style with undeifned type, given is "+type); return false; }
        styleLoad.setAttribute("type", "text/css");
        styleLoad.onload = function() {
        console.log("%cStylesheet from source "+hrefn+" is loaded dynamically successfully", "color:green;");styleLoad.onload = null;}
        styleLoad.onerror = function(){console.error("Cannot load stylesheet from source "+hrefn);}
        document.head.appendChild(styleLoad);
    }
    _self.injectScript = function(srcn, type="text/javascript", al=undefined, ann=false) {
        ...
        if(scriptLoad.readyState) {  // only required for IE <9
            scriptLoad.onreadystatechange = function() {
              if (scriptLoad.readyState==="loaded" || scriptLoad.readyState==="complete") {
                ...
                if(ann) roudlerInjectedScripts.push(scriptLoad);
    ...
    if(mainscript.textContent.length>0) { var innerConfig = mainscript.textContent.replace(/\s/g, "").split(";"), innerTemp = []; innerConfig.forEach(function(el, ind) {var ins = el.split("=");ins[0]=ins[0].replace("RoudlerConfig.", "");if(ins[0] in RoudlerConfig){innerTemp[ins[0]]=ins[1];}else if(ins[0]=="include"||ins[0]=="require"){preloadedModules[ins[0]]["other"].push(ins[1])}});console.log(innerTemp); /*for(var i=0;i<innerConfig;) {  }*/ mainscript.textContent = ""; }
    if(mainscript.hasAttribute("roudler-imports")) {var innerImports = mainscript.getAttribute("roudler-imports").replace("[","").replace("]","").split(",");for(var j=0;j<innerImports.length;j++)preloadedModules["include"].push(innerImports[j]);}
    if(mainscript.hasAttribute("roudler-requires")) {var innerRequires = mainscript.getAttribute("roudler-requires").replace("[","").replace("]","").split(",");for(var j=0;j<innerRequires.length;j++)preloadedModules["require"].push(innerRequires[j]);}
    ...
    if(checkES6()) {
        ...
        //Included Modules
        if(preloadedModules["include"].length>0){
            for(var i=0;i<preloadedModules["include"]["other"];i++) _self.injectScript(preloadedModules["include"]["other"][i]);
        }
        if(preloadedModules["require"].length>0){
            for(var i=0;i<preloadedModules["require"]["other"];i++) _self.injectScript(preloadedModules["include"]["other"][i]);
        }
    } else {
...
String.prototype.format = function() {var matchSet = this.match(/\$\{(([SNFB]|)\d+)\}/g),matchProc,newStr=this.toString();if(matchSet!=null && Array.isArray(matchSet)) {var inputArr = Array.prototype.slice.call(arguments);for(var i=0;i<matchSet.length;i++) {matchProc = matchSet[i].replace("${", "").replace("}", "");
